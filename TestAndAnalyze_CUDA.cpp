...
void buildCommandLine(char *commandline, const char* hostFileName,char *outputFileName,int threadNumber,const char *inputFileName,int tipoImagem,int blocosValue);
				...
				for(t=0;t<THREADS_NUM;++t){
					...
					*/
					...
					for(j=0;j<NUM_VEZES;++j){
						buildCommandLine(commandline,hostFileNames[b],arquivosAnalise[k-1],threadsValues[t],filenames[i],tipoImagens[i],blocosValues[b]);	
						// printf("%s\n", commandline);
						system(commandline);
					}
				}
...
void buildCommandLine(char *commandline, const char* hostFileName,char *outputFileName,int threadNumber,const char *inputFileName,int tipoImagem, int blocosValue){
	...
	*/
	...
	sprintf(strBloco, "%d", blocosValue);
	...
	if(threadNumber<0){
		...
		strcat(commandline,PARALLEL_NAME_CUDA);
		strcat(commandline," ");
		strcat(commandline,inputFileName);
		strcat(commandline," ");
		strcat(commandline,strTipoImg);
